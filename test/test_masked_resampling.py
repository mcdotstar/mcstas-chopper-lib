...
# Enough rays for the error bars to mean something and still under a few seconds.
NCOUNT = 400_000
...
def _compile(instr_path: Path, work: Path):
    ...
    binary, _ = mccode_compile(instr, work, flavor=Flavor.MCSTAS)
    runs = {}
    ...
    def run(**parameters):
        ...
        if key not in runs:
            ...
            runs[key] = subprocess.run(
                [str(binary), '-n', str(NCOUNT), '-s', str(SEED), '-d', str(directory), *settings],
